"""Push-token REST endpoints (B8).

- ``POST /api/casasmart/auth/push-token`` — register/refresh an FCM token
- ``DELETE /api/casasmart/auth/push-token`` — unregister on logout

Both are JWT-gated (any role can register their own token). The device_id
comes from the JWT claims, not the request body — a phone can only manage
its own push token.
"""
